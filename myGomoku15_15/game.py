...
class Board(object):
    ...
    def __init__(self, **kwargs):
        ...
        # mcts的动态搜索次数
        self.dynamic_n_playout = int(self.width * self.height * 1.3)
...
class Game(object):
    ...
    def start_supervised_play(self, filepath, current_qiju, mcts, is_show=True, temp=1e-3):
        self.board.init_board()
        p1, p2 = self.board.players
        states, mcts_probs, current_players = [], [], []
        current_step = 1
        print(filepath)
        while True:
            move, move_probs, is_last_step = sgf.get_file_step(self.board, filepath, index=current_step - 1, mcts=mcts,
                                                               n_playout=self.board.get_n_playout(),
                                                               temp=temp)
            # 存储数据
            states.append(self.board.current_state())
            ...
            self.board.update_n_playout(current_step=current_step)
            if is_show:
                print("当前是第:", current_qiju, "局， 当前是第：", current_step, "步, 为监督训练")
                graphic(self.board, p1, p2)
                pass
            current_step += 1
            end, winner = self.board.game_end()
